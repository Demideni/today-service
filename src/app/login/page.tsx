...
export default function Login() {
  ...
  async function onSubmit(e: React.FormEvent<HTMLFormElement>) {
    ...
    if (!res.ok) {
      setErr(mapErr(j.error || "Login failed"));
      return;
    }
    location.href = "/app";
  }
      ...
      <div className="max-w-md mx-auto space-y-4">
        ...
        <form className="bg-white border rounded-2xl p-4 space-y-3" onSubmit={onSubmit}>
          ...
          <label className="block">
            ...
            <input className="border rounded-xl px-3 py-2 w-full" name="password" placeholder="••••••••" type="password" required />
          </label>

          {err && <div className="text-sm text-red-600">{err}</div>}

          <button className="w-full bg-blue-600 text-white rounded-xl py-2 font-medium">Sign in</button>
